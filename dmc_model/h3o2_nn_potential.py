...
model.load_state_dict(torch.load('h3o2_nn_model_molec_sorted_1.5mill_0_reg_90hidden_bn.pth',map_location=torch.device('cpu')))
...
def calculate_feature_vectors(molecule_coords, atomic_numbers):
    """
    Calculate the feature vectors using the lower triangle of the molecule sorted Coulomb matrix for OH-(H2O) (aka h3o2).
    
    Parameters:
    molecule_coords (np.ndarray): An array of shape (n_molecules, 5, 3) containing the Cartesian coordinates.
    atomic_numbers (np.ndarray): An array of shape (5,) containing the atomic numbers.

    Returns:
    np.ndarray: An array of input features for the nn model of shape (n_molecules, 10).
    """
    ...
    groups = [[0],[1,2],[3,4]] #assuming the input geometries list the shared proton first
...
def cart_to_pot(cds):
    ...
    Parameters:
    cds (np.ndarray): An array of shape (n_molecules, 5, 3) containing the Cartesian coordinates in Bohr.

    Returns:
    np.ndarray: An array shape (n_molecules, ) of potenial energies in a.u.
    """
    atoms = np.array([1,8,1,8,1])
