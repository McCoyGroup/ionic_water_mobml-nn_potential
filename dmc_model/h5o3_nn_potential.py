...
model.load_state_dict(torch.load('h5o3_nn_model_molec_atom_sorted_1.5mill_0_reg_360hidden_bn.pth',map_location=torch.device('cpu')))
...
def calculate_feature_vectors(molecule_coords, atomic_numbers):
    """
    Calculate the feature vectors using the lower triangle of the molecule + atom sorted Coulomb matrix for OH-(H2O)2 (aka h5o3).
    
    Parameters:
    molecule_coords (np.ndarray): An array of shape (n_molecules, 8, 3) containing the Cartesian coordinates.
    atomic_numbers (np.ndarray): An array of shape (8,) containing the atomic numbers.

    Returns:
    np.ndarray: An array of input features for the nn model of shape (n_molecules, 28).
    """
    ...
    groups = [[0,1],[2,3,4],[5,6,7]] #assuming the hydroxide ion is listed first in the coordinates
...
def cart_to_pot(cds):
    """
    Calculates the potential energies of input geometries for OH-(H2O)2 (aka h5o3) in a.u.
    for use in DMC simulations

    Parameters:
    cds (np.ndarray): An array of shape (n_molecules, 8, 3) containing the Cartesian coordinates in Bohr.

    Returns:
    np.ndarray: An array shape (n_molecules, ) of potenial energies in a.u..
    """
    atoms = np.array([8,1,8,1,1,8,1,1])
